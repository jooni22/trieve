...
### SLEEP FOR WAITING TO SSL GENERATION
sleep 3

if sudo journalctl -u caddy | grep -ni "error"; then
    echo "Error: Error in caddy service:"
    sudo journalctl -u caddy | grep -ni "error\|warn"
    echo "If you reach limit try add the staging Let's Encrypt server to /etc/caddy/Caddyfile:"
